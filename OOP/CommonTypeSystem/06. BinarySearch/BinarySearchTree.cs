namespace _06.BinarySearch
{
     ...
     where T : IComparable<T>
    {
        ...
        {
            TreeNode<T> nodeFound = this.Find(item);
        ...
        {
            TreeNode<T> nodeToDelete = Find(item);
        ...
        private void Remove(TreeNode<T> node)
        {
            ...
            {
                TreeNode<T> replacement = Min(node.Right);
                node.Item = replacement.Item;
                node = replacement;
            }

            // Case 1 and 2: If the node has at most one child
            TreeNode<T> theChild = node.Left != null ? node.Left : node.Right;
        ...
        private void AsStringAscending(TreeNode<T> node)
        {
            if (node != null)
            {
        ...
        private void AsStringDescending(TreeNode<T> node)
        {
            if (node != null)
            {
        ...
        private bool AreEqual(TreeNode<T> a, TreeNode<T> b)
        {
            if (a == null && b == null)
            {
                return true;
        ...
        private void CalcHashCodePreorder(TreeNode<T> node, ref int hashCode)
        {
            if (node != null)
            {
